// import React, { useEffect, useState } from 'react';
// import { fetchChats } from '../../utils/ChatAPI';
// import { useSelector } from 'react-redux';
// import Loader2 from '../../components/Loaders/Loader2'
// import { NavLink } from 'react-router-dom';

// function ChatsList() {

//   const currentUserId = useSelector((state) => state.userData?.currentUserData?.data?._id)
//   const [chats, setChats] = useState([])
//   const [userToChat, setUserToChat] = useState(false)

//   const handleFetchChats = async () => {
//     const response = await fetchChats()
//     setChats(response?.data.data)
//     console.log("chatList",response?.data.data)
//   }

//   useEffect(() => {
//     handleFetchChats()
//   }, []);


//   //getting the otherUser , to whom the currUser Is Chatting
//   useEffect(() => {
//     const otherUsers = [];

//     for (let chat of chats) {
//       const user = chat.users.find((u) => u._id !== currentUserId);
//       if (user) {
//         otherUsers.push(user);
//       }
//     }

//     setUserToChat(otherUsers);
//   }, [chats, currentUserId]);


//   if (userToChat) {
//     console.log("user to chat", userToChat)
//   }


//   return (
//     <>
//       <div className='py-13 flex flex-col xl:w-[80vw] xl:absolute right-0 xl:justify-center xl:items-center p-3 mt-2'>

//         <div className='flex flex-col gap-1 items-center'>
//           {
//             userToChat ? (
//               userToChat.map((data, index) => (
//                 <div key={index} className='border-1 border-gray-500 w-full md:w-[70vw] xl:w-[40vw]'>
//                   <NavLink to={`/chat/messages/${data._id}`} className="flex items-center p-1 rounded-xl h-25">
//                     <div className='w-25 h-25 flex items-center justify-center'>
//                       <img src={data.profilePicture} alt="" className='h-23 w-23 rounded-full' />
//                     </div>
//                     <div className='flex flex-col h-full p-4 font-[400]'>
//                       <p className='text-[18px]'>{data.name}</p>
//                     </div>
//                   </NavLink>
//                 </div>
//               ))
//             ) : (
//               <Loader2 />
//             )

//           }
//         </div>
//       </div>
//     </>
//   );
// }

// export default ChatsList;





import React, { useEffect, useState } from 'react';
import { fetchChats } from '../../utils/ChatAPI';
import { useSelector } from 'react-redux';
import Loader2 from '../../components/Loaders/Loader2'
import { NavLink } from 'react-router-dom';

function ChatsList() {

  const currentUserId = useSelector((state) => state.userData?.currentUserData?.data?._id)
  const [chats, setChats] = useState([])

  const handleFetchChats = async () => {
    const response = await fetchChats()
    setChats(response?.data.data)
    console.log("chatList", response?.data.data)
  }

  useEffect(() => {
    handleFetchChats()
  }, []);

  return (
    <>
      <div className='py-13 flex flex-col xl:w-[80vw] xl:absolute right-0 xl:justify-center xl:items-center p-3 mt-2'>

        <div className='flex flex-col gap-1 items-center'>
          {
            chats.map((data, index) => {
              const otherUser = data.users.find((user) => user._id !== currentUserId);
              return (
                <div key={index} className='border-1 border-gray-500 w-full md:w-[70vw] xl:w-[40vw]'>
                  <NavLink to={`/chat/messages/${data._id}`} className="flex items-center p-1 rounded-xl h-25">
                    <div className='w-25 h-25 flex items-center justify-center'>
                      <img
                        src={otherUser?.profilePicture}
                        alt={otherUser?.name}
                        className='h-23 w-23 rounded-full object-cover'
                      />
                    </div>
                    <div className='flex flex-col h-full p-4 font-[400]'>
                      <p className='text-[18px]'>{otherUser?.name}</p>
                      <p className='text-gray-500 text-[14px] truncate'>
                        {data?.latestMessage?.content || "Start your conversation..."}
                      </p>
                    </div>
                  </NavLink>
                </div>
              );
            })

          }
        </div>
      </div>
    </>
  );
}

export default ChatsList;





